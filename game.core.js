...
var starting_pos_other = {x:448, y:starting_y_pos};
...
/* The game_core class */
	...
	var y_offset = 180, x_offset = 105, y_normal=720/380, x_normal=3.6;
	y_normal*=0.7;
	
	this.obstacles = [
		new wall_obstacle({x:(50+x_offset)*x_normal, y:this.world.height-(-135+y_offset)*y_normal}, 10*x_normal , 40*y_normal),
		new wall_obstacle({x:(-50+x_offset)*x_normal, y:this.world.height-(-135+y_offset)*y_normal}, 10*x_normal , 40*y_normal),
		new wall_obstacle({x:(0+x_offset)*x_normal, y:this.world.height-(-135+y_offset)*y_normal}, 10*x_normal , 40*y_normal),
		new wall_obstacle({x:(25+x_offset)*x_normal, y:this.world.height-(-60+y_offset)*y_normal}, 10*x_normal , 40*y_normal),
		new wall_obstacle({x:(-25+x_offset)*x_normal, y:this.world.height-(-60+y_offset)*y_normal}, 10*x_normal , 40*y_normal),
		new wall_obstacle({x:(75+x_offset)*x_normal, y:this.world.height-(-60+y_offset)*y_normal}, 10*x_normal , 40*y_normal),
		new wall_obstacle({x:(-75+x_offset)*x_normal, y:this.world.height-(-60+y_offset)*y_normal}, 10*x_normal , 40*y_normal),
		new wall_obstacle({x:(50+x_offset)*x_normal, y:this.world.height-(15+y_offset)*y_normal}, 10*x_normal , 40*y_normal),
		new wall_obstacle({x:(-50+x_offset)*x_normal, y:this.world.height-(15+y_offset)*y_normal}, 10*x_normal , 40*y_normal),
		new wall_obstacle({x:(20+x_offset)*x_normal, y:this.world.height-(115+y_offset)*y_normal}, 10*x_normal , 40*y_normal),
		new wall_obstacle({x:(-20+x_offset)*x_normal, y:this.world.height-(115+y_offset)*y_normal}, 10*x_normal , 40*y_normal),
		new wall_obstacle({x:(20+x_offset)*x_normal, y:this.world.height-(155+y_offset)*y_normal}, 10*x_normal , 40*y_normal),
		new wall_obstacle({x:(-20+x_offset)*x_normal, y:this.world.height-(155+y_offset)*y_normal}, 10*x_normal , 40*y_normal),
		new wall_obstacle({x:(0+x_offset)*x_normal, y:this.world.height-(15+y_offset)*y_normal}, 10*x_normal , 40*y_normal),
		new wall_obstacle({x:(75+x_offset+23)*x_normal, y:this.world.height-(0+y_offset+15)*y_normal}, 56*x_normal , 10*y_normal),
		new wall_obstacle({x:(-75+x_offset+23)*x_normal, y:this.world.height-(0+y_offset+15)*y_normal}, 56*x_normal , 10*y_normal),
		new wall_obstacle({x:(60+x_offset+35)*x_normal, y:this.world.height-(100+y_offset+15)*y_normal}, 80*x_normal , 10*y_normal),
		new wall_obstacle({x:(-60+x_offset+35)*x_normal, y:this.world.height-(100+y_offset+15)*y_normal}, 80*x_normal , 10*y_normal)
	];
...
game_core.prototype.v_add = function(a,b) { return { x:(a.x+Math.sin(b.x)*playerspeed).fixed(), y:(a.y+Math.cos(b.x)*playerspeed).fixed() }; };
	//rotation only
game_core.prototype.v_add2 = function(a,b) { return { x:(a.x+Math.sin(b)*playerspeed).fixed(), y:(a.y+Math.cos(b)*playerspeed).fixed() }; };
...
//        game.ctx.fillText(this.state, this.pos.x+10, this.pos.y + 4);
   ...
   wall_obstacle.prototype.draw = function(){
	     //Set the color for this obstacle
        game.ctx.fillStyle = this.color;

            //Draw the rectangle
        game.ctx.fillRect(this.left, this.up, this.width, this.height);
...
game_core.prototype.check_collision = function( item ) {
        ...
        //Left wall.
    if(item.pos.x <= item.pos_limits.x_min && Math.sin(item.x_dir)<=0) {
        //item.pos.x = item.pos_limits.x_min;
	item.x_dir= - item.x_dir;
    }

        //Right wall
    if(item.pos.x >= item.pos_limits.x_max && Math.sin(item.x_dir)>=0) {
        //item.pos.x = item.pos_limits.x_max;
	item.x_dir= - item.x_dir;
    }
    
        //Roof wall.
    if(item.pos.y <= item.pos_limits.y_min && Math.cos(item.x_dir)<=0 && this.server) {
        //item.pos.y = item.pos_limits.y_min;
	this.server_reset(item);
	//item.x_dir=Math.PI- item.x_dir;
    }

        //Floor wall
    if(item.pos.y >= item.pos_limits.y_max && Math.cos(item.x_dir)>=0) {
	...
	if (item.pos.x+item.size.hx>=this.obstacles[i].left && item.pos.x+item.size.hx<=this.obstacles[i].right && item.pos.y
		+item.size.hy<this.obstacles[i].down && item.pos.y>this.obstacles[i].up && Math.sin(item.x_dir)>=0)
	 	item.x_dir = - item.x_dir;
	//collision on right side of obstacle
	if (item.pos.x>=this.obstacles[i].left && item.pos.x<=this.obstacles[i].right && item.pos.y
		+item.size.hy<this.obstacles[i].down && item.pos.y>this.obstacles[i].up && Math.sin(item.x_dir)<=0)
	 	item.x_dir = - item.x_dir;
...
game_core.prototype.process_input = function( player ) {
    ...
    if(ic) {
        for(var j = 0; j < ic; ++j) {
            ...
            for(var i = 0; i < c; ++i) {
                var key = input[i];
                if(key == 'l') {
			player.x_dir-=turningSpeed*Math.PI/180;
                    x_dir -= 1;
                }
                if(key == 'r') {
			player.x_dir +=turningSpeed*Math.PI/180;
    ...
    var resulting_vector = this.physics_movement_vector_from_direction(player.x_dir,y_dir);
...
game_core.prototype.server_update_physics = function() {
    ...
    this.players.self.pos = this.v_add2( this.players.self.old_state.pos, this.players.self.x_dir );
    ...
    this.players.other.pos = this.v_add2( this.players.other.old_state.pos, this.players.other.x_dir);
...
game_core.prototype.server_update = function(){
	...
	//sxd : this.players.self.x_dir,		    // 'host direction'
	//oxd : this.players.other.x_dir,		    // ' direction'
...
game_core.prototype.server_reset = function(item){
	var win = (item==this.players.self) ?  1 : 0;
	if (win==1)
		this.players.self.score++;
	else
		this.players.other.score++;

	this.players.self.pos = starting_pos_self;
        this.players.other.pos = starting_pos_other;
        
        this.players.self.x_dir = this.players.other.x_dir = Math.PI;
        this.players.self.instance.emit('reset', (win==1)?1:0)
        this.players.other.instance.emit('reset', (win==0)?1:0)
	
}; //game_core.server_reset
...
game_core.prototype.client_handle_input = function(){
    ...
    if( this.keyboard.pressed('A') ||
        this.keyboard.pressed('left')) {
            ...
            x_dir = -1;
		this.players.self.x_dir -=turningSpeed*Math.PI/180;
            input.push('l');
...
game_core.prototype.client_update_physics = function() {
	...
	this.players.self.cur_state.pos = this.v_add2( this.players.self.old_state.pos, this.players.self.x_dir);
...
game_core.prototype.client_connect_to_server = function() {
		...
		//handle endgame
	this.socket.on('reset', this.client_handle_endgame.bind(this));

}; //game_core.client_connect_to_server

game_core.prototype.client_handle_endgame = function (win){
		if (win==1) this.players.self.score++;
		else this.players.other.score++;
		this.client_update_score();
	}
...
game_core.prototype.client_update_score = function(){
	this.ctx.fillStyle = 'rgba(255,255,255,0.7)';
	this.ctx.fillText ( 'You: ' + this.players.self.score + ' Opponent: ' + this.players.other.score, 300, 465);
}
